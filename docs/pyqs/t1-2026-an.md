...
Every question below is the paper's own text. Code that appears as images in the PDF is written out — where the exact values matter and the image was unreadable, the snippet is marked *(reconstructed)*. Attempt each question, then open the answer.
...
When committing code to a public repository, which files should be added to `.gitignore`? *(the four options are filenames shown as images; two are correct)*

- **A.** `.env` — environment variables with API keys
- **B.** `README.md` — project documentation
- **C.** `secrets.json` — stored credentials
- **D.** `requirements.txt` — dependency list
...
Your UI on `localhost:3000` fetches from `localhost:8000`. The browser throws a CORS error. How to resolve it? *(URLs reconstructed)*
...
Why pin dependencies (e.g., `pandas==2.2.0`) instead of leaving them unpinned? *(version reconstructed)*
...
Fraud model: 99.2% test accuracy, 52% production accuracy. Dataset chronological, `shuffle=True` in the split, rolling stats computed **before** splitting. What caused the collapse? *(split option reconstructed)*
...
A Vercel edge function processes logs; needs 40 seconds but hits its limit at 10 seconds. Why?
...
To fix this, the student adds an explicit `Cache-Control` parameter directly to the response header. The application speeds up drastically. However, when the student tries to test the API locally by fetching from the React UI (`localhost:5173`) to the backend (`localhost:8000`), the browser abruptly rejects the network connection and throws an error indicating missing headers. *(ports reconstructed)*
...
During load testing, the backend evaluates 5 web requests concurrently using asynchronous execution (`asyncio.gather`). However, the entire server process completely halts when one specific downstream node hangs randomly for 60 seconds without resolving. Finally, a malicious user inputs trick instructions via the "Movie Title" input field, attempting to cleanly override the LLM's primary instructions.
...
Why does the browser reject the connection when React (`localhost:5173`) fetches from FastAPI (`localhost:8000`)?
...
*A Dockerfile and backend structure were shown as images in the paper.*
...
After moving the application to Docker with `EXPOSE 8501` in the Dockerfile, running it fails to connect at `localhost:8501`. What is missing?
